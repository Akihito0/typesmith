...
function traceFrame(
  ...
  const x = (frame.x - origin.x) * scale;
  const y = (frame.y - origin.y) * scale;
  ...
  const radius = Math.min(frame.radius * scale, width / 2, height / 2);
  if (radius <= 0) {
    ctx.rect(x, y, width, height);
    return;
  }
  // Built by hand rather than via roundRect so older canvas implementations
  // still get the corners.
...
export async function exportPlaygroundPng(
  ...
  ctx.save();
  if (frame) {
    // Text is clipped to the frame it sits on, matching the on-canvas crop.
    traceFrame(ctx, frame, origin, scale);
    ctx.clip();
  }
